...
def render_trend_tab(router: ModelRouter, config: dict):
    ...
    values_radar = [
        ...
        int(benchmarks["margin_range"].split("-")[1].replace("%", "")) * 2,
        100 - benchmarks["top_competitors"] * 5,
        {"Low": 90, "Low-Medium": 70, "Medium": 50, "Medium-High": 30, "High": 10}.get(benchmarks["entry_difficulty"], 50),
        min(int(benchmarks["market_size"].replace("$", "").replace("B", "")) * 10, 100),
    ]
    fig = go.Figure(data=go.Scatterpolar(r=values_radar, theta=categories_radar, fill="toself", name=config.get("sub_category", "")))
    ...
    with col1:
        time_horizon = st.selectbox("时间跨度", ["3m", "6m", "12m"],
                                    format_func=lambda x: {"3m": "3个月", "6m": "6个月", "12m": "12个月"}.get(x, x))
    with col2:
        report_depth = st.selectbox("报告深度", ["executive", "full"],
